...
$db->setQuery("Select * from #__contact_details WHERE published = 1 order by name");
...
<?php
foreach ($data as $unContacto) {
    $unContacto['params'] = json_decode($unContacto['params']);

    $ubicaciones[] = array(
        "0" => $unContacto['name'],
        "1" => $unContacto['params']->contact_latitud,
        "2" => $unContacto['params']->contact_longitud,
        "3" => JRoute::_(ContactHelperRoute::getContactRoute($unContacto['id'] . ":" . $unContacto['alias'], $unContacto['catid'])),
        "4" => $unContacto['catid'],
        "5" => $unContacto['params']->contact_horario,
        "6" => $unContacto['telephone'],
        "7" => $unContacto['address'],
        "8" => $unContacto['email_to'],
        "9" => $unContacto['id'],
    );
    $provinciaCanton[$unContacto['state']][] = $unContacto['suburb'];
    ksort($provinciaCanton[$unContacto['state']]);
}
ksort($provinciaCanton);
?>
    <script
        src="https://maps.googleapis.com/maps/api/js?key=AIzaSyD-tjUU-RaVLhHFtSa5oSKJUNHvNuANxJM&language=es&region=CR"></script>
<div class="ui raised segment container">
    <?php echo JLayoutHelper::render('joomla.content.categories_default', $this); ?>
    <div class="ui secondary segment ">
	<div class="ui three column grid container stackable categories-list<?php echo $this->pageclass_sfx; ?>">
        <?php
...
if (!empty($ubicaciones)) {
            ...
            <div class="ui filters toolbar container">
                ...
                <?php if ($this->params->get('filter_field') != 'hide') : ?>
                    <div class="three fields">
                        ...
                        <div class="field">
                            <label class="filter-search-lbl element-invisible" for="filtroCanton">Localidad</label>
                            <select disabled class="ui fluid search dropdown filtros" id="filtroCanton">
    ...
    <div class="ui three doubling stackable cards special ">
        ...
        <div class="ui fluid card oficina"
             data-provincia="<?php echo $unContacto['state']; ?>"
             data-canton="<?php echo $unContacto['suburb']; ?>"
             data-latitud="<?php echo $unContacto['params']->contact_latitud ?>"
             data-longitud="<?php echo $unContacto['params']->contact_longitud ?>"
             data-name="<?php echo $unContacto['name']; ?>"
             data-telephone="<?php echo $unContacto['telephone']; ?>"
             data-horario="<?php echo $unContacto['params']->contact_longitud; ?>"
             data-email_to="<?php echo $unContacto['email_to']; ?>"
             data-id="<?php echo $unContacto['id']; ?>"
             data-addres="<?php echo $unContacto['address']; ?>"
             data-url="<?php echo JRoute::_(ContactHelperRoute::getContactRoute($unContacto['id'] . ":" . $unContacto['alias'], $unContacto['catid'])); ?>"
             data-catid="<?php echo $unContacto['catid']; ?>">
            <div class="content">
                <div class="header">
                    <?php echo $unContacto['name']; ?>
                </div>
                <div class="meta"><b></b>
                </div>
            </div>
            <div class="content">
                <div class="description">
                    <span class="telephone"><i
                            class="ui phone icon"> </i>Teléfono: <b><?php echo $unContacto['telephone']; ?></b></span><br/>
                    <?php if (!empty($unContacto['mobile'])){ ?>
                    <span class="telephone"><i
                            class="ui phone icon"> </i>Teléfono: <b><?php echo $unContacto['mobile']; ?></b></span><br />
                    <?php } ?>
                    <span class="fax">      <i
                            class="ui fax icon"> </i>Fax: <?php echo $unContacto['fax']; ?> </span><br>
                    <span class="email">    <i class="ui mail icon"> </i><a
                            href="<?php echo $unContacto['email_to']; ?>"><?php echo $unContacto['email_to']; ?></a></span><br>
                    <span class="address"><?php echo $unContacto['suburb'] . " " . $unContacto['state']; ?></span>
                </div>
            </div>
            <div class="extra center aligned content">

                <a class="ui basic green button"
                   href="<?php echo JRoute::_(ContactHelperRoute::getContactRoute($unContacto['id'] . ":" . $unContacto['alias'], $unContacto['catid'])); ?>"><i
...
?>
    ...
    function filtroProvincia() {
        if (jQuery("#filtroProvincia").val() == "0") {
            ...
            jQuery('#filtroCanton').attr('disabled','disabled');
            jQuery('.oficina').show();
            initMap(ubicaciones);
        }
        else {
            ...
            jQuery("#filtroCanton").removeAttr('disabled');
            jQuery('.oficina').show();
