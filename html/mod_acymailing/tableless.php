...
<?php
	...
	<div class="acymailing_fulldiv" id="acymailing_fulldiv_<?php echo $formName; ?>" <?php echo $styleString; ?> >
		<form class="ui form" id="<?php echo $formName; ?>" action="<?php echo JRoute::_('index.php'); ?>" onsubmit="return submitacymailingform('optin','<?php echo $formName;?>')" method="post" name="<?php echo $formName ?>" <?php if(!empty($fieldsClass->formoption)) echo $fieldsClass->formoption; ?> >
		<div class="acymailing_module_form" >
			<?php if(!empty($introText)) echo '<div class="acymailing_introtext">'.$introText.'</div>';

			$listContent = '';
			if($params->get('dropdown',0)){
				$listContent .= '<div class="ui dropdown selection"><select name="subscription[1]">';
			...
			<div class="acymailing_form">
					...
					foreach($fieldsToDisplay as $oneField){
						if(empty($extraFields[$oneField])) echo '<div class="one field fieldacy'.$oneField.'" id="field_'.$oneField.'_'.$formName.'">';
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							if($extraFields[$oneField]->type == 'category'){
								...
								echo '<div class="one field fieldacy'.$oneField.'" id="field_'.$oneField.'_'.$formName.'">';
								if($displayOutside){
				...
				<?php }
				 ...
				 if($params->get('showterms',false)){
					...
					<p class="acysubbuttons">
						...
						<button class="ui green button subbutton" type="submit" name="Submit" onclick="try{ return submitacymailingform('optin','<?php echo $formName;?>'); }catch(err){alert('The form could not be submitted '+err);return false;}"><?php $subtext = $params->get('subscribetextreg'); if(empty($identifiedUser->userid) OR empty($subtext)){ $subtext = $params->get('subscribetext',JText::_('SUBSCRIBECAPTION')); } echo $subtext;  ?></button>
						...
						<button class="ui teal button unsubbutton" type="button" name="Submit" onclick="return submitacymailingform('optout','<?php echo $formName;?>')"><?php echo $params->get('unsubscribetext',JText::_('UNSUBSCRIBECAPTION')); ?></button>
						<?php } ?>
					</p>
				</div>
			<?php
