...
if(count($list)>0){?>
<div id="btcontentslider<?php echo $module->id; ?>" style="display:none;width:<?php echo $moduleWidthWrapper;?>" class="bt-cs<?php echo $moduleclass_sfx? ' bt-cs'.$params->get('moduleclass_sfx'):'';?> newsBox">
	...
	?>
	<h2 style="width:<?php echo $moduleWidth;?>">
	...
	<?php } ?>
        ...
        <div class="slides_container" style="width:<?php echo $moduleWidth.";".$add_style;?>">
            <?php foreach( $pages as $key => $list ): ?>
                <div class="slide " style="width:<?php echo $moduleWidth;?>">
                    <div class="ui four column doubling  divided stackable grid">
                    <?php foreach( $list as $i => $row ): ?>

                        <div class="bt-row column <?php if($i==0) echo 'bt-row-first'; else if($i==count($list)-1) echo 'bt-row-last' ?>"  style="width:<?php echo $itemWidth;?>%" >
                            <div class="bt-inner">
                                ...
                                <?php if( $row->thumbnail && $align_image == "center" ): ?>
                                    ...
                                    <div class="bt-center"  >
                                        <div style="max-width:360px; margin: auto;" class="effect2">
                                    <a target="<?php echo $openTarget; ?>"
                                        ...
                                        <img  src="<?php echo $row->thumbnail; ?>" alt="<?php echo $row->title?>"  style="width:<?php echo $thumbWidth ;?>px;" title="<?php echo $row->title?>" />
                                            ...
                                            <?php if( $showReadmore ) : ?>
                                                <div class="ui middle aligned readmore">
                                                    <a target="<?php echo $openTarget; ?>"
                                                       title="<?php echo $row->title;?>"
                                                       href="<?php echo $row->link;?>"> <?php echo JText::_('Ver Noticia');?>
                                                    </a>
                                                </div>
                                            <?php endif; ?>
                                        </div>
                                ...
                                <?php if( $showAuthor || $showDate ): ?>
                                    <div class="bt-extra" style="max-width:360px; margin: auto;">
                                        <?php if( $showAuthor ): ?>
                                        ...
                                        <?php endif; ?>
                                        <?php if( $showDate ): ?>
                                            <span class="bt-date"><i class="ui calendar icon"></i> <?php echo JText::_('JGLOBAL_CREATED_DATE_ON'); ?>
                                                <?php
                                                $fechalarga = $row->date;
                                                $fechacorta = substr($fechalarga, 0, -5);
                                                echo($fechacorta);
                                                ?>
                                        </span>
                                        <?php endif; ?>
                                ...
                                <!-- News Title -->
                                <div style="max-width:360px; margin: auto; text-align: left;">
                                    <?php if( $showTitle ): ?>
        ...
        <?php if( $next_back && $totalPages  > 1  ) : ?>
            <a class="prev" href="#"><i class="angle double left huge icon"></i></a>
            <a class="next" href="#"><i class="angle double right huge icon"></i></a>
        <?php endif; ?>
</div>
<!--end bt-container -->
<div style="clear: both;"></div>
