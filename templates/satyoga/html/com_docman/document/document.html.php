...
<div class="docman_document" itemscope itemtype="http://schema.org/CreativeWork">
        ...
        <span class="koowa_header__item koowa_header__item--image_container">
            ...
            <a class="koowa_header__image_link <?= $params->document_title_link === 'download' ? 'docman_track_download' : ''; ?>"
               ...
               <?= $params->download_in_blank_page && $params->document_title_link === 'download' ? 'target="_blank"' : ''; ?>><!--
                -->
                <?= import('com://site/docman.document.icon.html', array('icon' => $document->icon, 'class' => ' koowa_icon--24')); ?>
            </a>
            <? else: ?>
                <?= import('com://site/docman.document.icon.html', array('icon' => $document->icon, 'class' => ' koowa_icon--24')); ?>
            <? endif; ?>
    ...
    if ( in_array($document->extension, array("mp4","m4v","flv","mov","f4v","webm"))){
        if (in_array($document->extension, array("mp4","m4v","fv4","mov"))){
            $type = "mp4";
            ?>
            <video width="320" height="240" controls>
                <source src="<? echo $document->download_link; ?>" type="video/<? echo $type;?>">
    ...
    //audio
    if (in_array($document->extension, array("webm","ogg","mp3","wave","wav","x-wav"))){
        ?>
        <audio width="320" controls>
            <source src="<?echo $document->download_link;?>" type="audio/<? echo $document->extension ?>" >
        </audio>
        <?
    }
    ?>
